...
El **monto se escribe por alumna** en la lista de convocadas (varía por torneo y a veces por alumna). El monto cargado a una se **sugiere** en las casillas vacías de las demás, en gris: la sugerencia no se guarda por aparecer — queda confirmada si se la edita, si se toca Enter sobre ella, o al marcar la inscripción paga o con recargo —, y nunca pisa un monto ya cargado. El **recargo** es un botón al lado del monto que se prende a mano (`recargo_aplicado`), como en cuotas: no se aplica solo por fecha. Suma `RECARGO_MONTO`, la misma constante de Pagos; se guarda el booleano y no el valor, así que si la constante cambia, cambia también en las inscripciones ya marcadas. La planilla que se baja lleva la columna **Monto** (con el recargo desglosado, «60.000 + 10.000») y el **total al pie** de lo pagado — solo para quien ve la plata; la de la Profesora sale sin esa columna.
...
En Deudores, las bajas van **agrupadas al final**, tintadas en rojo y con el badge «Baja», y conservan **Cobrar** y **Saldar sin cobrar** — son la única vía para cerrarles la deuda; «Cobrar» las abre resueltas aunque no figuren en el buscador de alta, que es de activas. Una vez saldadas desaparecen de la lista. El resto de la lista va en **orden alfabético** por apellido y nombre (sin distinguir tildes ni mayúsculas), el mismo criterio que Pendientes de verificar.
