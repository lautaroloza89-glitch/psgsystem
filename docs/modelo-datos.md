...
> Extendido el 2026-09-10 (Correcciones post-rediseño, Bloque 2): `supabase/migrations/20260910120000_notificaciones_purga_7_dias.sql` agrega la función `purgar_notificaciones_viejas()` y el job de pg_cron `purgar_notificaciones_viejas_diario`, que borra de `notificaciones` todo lo que pasó los 7 días desde `creado_en`. Ver detalle en la sección `notificaciones` más abajo.
> Extendido el 2026-09-10 (Correcciones post-rediseño): `supabase/migrations/20260910130000_turnos_grupo_fecha_tipo_unica.sql` agrega la constraint única `turnos_grupo_fecha_tipo_unica` sobre (`grupo_id`, `fecha`, `tipo`) — la base aceptaba dos filas de Patín (o dos de física) para la misma fecha del mismo grupo, y las pantallas mostraban una y escondían la otra en silencio. Ver detalle en la sección `turnos` más abajo.
...
- **Head Coach ve y edita todas las tareas desde el 2026-09-08** (`supabase/migrations/20260908160000_head_coach_ve_y_edita_todas_las_tareas.sql`). `tareas_select` y la policy de update —renombrada a `tareas_update_gestion_y_profesor`, porque el nombre viejo ya no decía quién entra— pasan a `Admin / Secretaria / Head Coach` sin condición, más Profesor sobre lo propio o asignado. Antes Head Coach dependía de `tarea_visible_para_head_coach(uuid)`, que la habilitaba sobre las tareas creadas por (o asignadas a) Profesor, Empleado y Patinador: quedaba afuera una tarea que fuera solo entre Admin y Secretaria. **La lectura se amplió junto con la escritura** a propósito: abrir solo el `update` dejaría a Head Coach editando algo que ningún listado le muestra. `tareas_delete` no se tocó. La función `tarea_visible_para_head_coach` **quedó sin uso en las policies de `tareas`** y no se dropeó todavía (es `SECURITY DEFINER`, conviene confirmar que no la use nada más) — anotado en `docs/pendientes.md`.
...
**Una fecha de un grupo puede tener dos filas** (2026-09-09): una de `tipo = 'Patín'` y otra de `'Preparación física'`. Un grupo como Avanzado entrena 19:30–22:00 con la media hora inicial de física — es una sola clase real y un solo horario (`grupo_horarios` sigue con un bloque por día), pero son dos contenidos de planificación distintos, cada uno con su profesora en `turno_profesores`. No hay constraint de unicidad por `(grupo_id, fecha)` y nunca la hubo: el modelo siempre lo permitió. Lo que fallaba era el guardado — `upsertPlanificacionFecha` buscaba la fila existente sin mirar `tipo`, así que cargar la física **pisaba** la planificación de patín de esa fecha y la primera se perdía de la base. Corregido en `main` sin migración (la columna `tipo` ya existía); el mismo filtro hizo falta en el redirect de `duplicarPlanificacion`, cuyo `.single()` fallaba con las dos filas cargadas. Quien lee `turnos` por grupo+fecha tiene que contar **fechas distintas** y no filas: el conteo mensual de Planificaciones lo hace así, y la física va en un renglón aparte sin denominador.

**Constraint única `turnos_grupo_fecha_tipo_unica`** (`grupo_id`, `fecha`, `tipo`), agregada el 2026-09-10 (`supabase/migrations/20260910130000_turnos_grupo_fecha_tipo_unica.sql`). Un grupo entrena una vez por fecha y en esa franja puede haber Patín, Preparación física, o las dos — dos filas con distinto `tipo`, mismo grupo, misma fecha, mismo horario. Lo que no tiene sentido es que haya **dos de Patín** o **dos de física** para la misma fecha del mismo grupo, y hasta acá nada lo impedía: el arreglo del 2026-09-09 (`upsertPlanificacionFecha` mirando `tipo`) vive en la aplicación, y la base seguía aceptando el duplicado por cualquier otro camino. El duplicado además no se ve: las pantallas que arman un mapa por fecha muestran una fila y esconden la otra en silencio. Verificado antes de aplicar: 0 duplicados en producción. `grupo_id` es nullable y en Postgres los NULL no colisionan entre sí en un índice único, así que los turnos legacy sin grupo quedan fuera de la regla — correcto, porque sin grupo no existe «el turno de ese grupo en esa fecha».
...
**Retención: 7 días** (2026-09-10, Correcciones post-rediseño Bloque 2 — `supabase/migrations/20260910120000_notificaciones_purga_7_dias.sql`). La tabla no tenía ninguna política de retención: crecía para siempre y el ícono de campana mostraba el histórico entero. Se agrega la función `public.purgar_notificaciones_viejas()` (security definer, `delete from notificaciones where creado_en < now() - interval '7 days'`) y el job de pg_cron `purgar_notificaciones_viejas_diario` corriendo `0 5 * * *` (05:00 UTC = 02:00 Argentina, fuera de horario de uso). Mismo mecanismo que `notificar_tareas_vencimiento_diario`, sobre la extensión ya instalada — no hace falta infraestructura nueva.

- **No distingue `leida`**: a los 7 días una notificación sin leer tampoco aporta nada, y el evento que la originó sigue vivo en su módulo (la tarea, la clase, el comentario).
- **Efecto sobre el dedup de vencimientos**: `notificar_tareas_vencimiento()` evita repetir un aviso chequeando que no exista ya una fila con el mismo `(tarea_id, usuario_id, tipo)`. Al borrarse esa fila a los 7 días, una tarea que sigue vencida y sin completar vuelve a generar su aviso de `'tarea_vencida'` (y su push) en el chequeo diario siguiente — aceptado a propósito: una tarea vencida hace más de una semana y todavía sin tocar merece que el aviso reaparezca. `'tarea_por_vencer_2d'` y `'tarea_por_vencer_1d'` no se repiten por este camino: cuando la fila cumple 7 días, la tarea ya pasó a `'tarea_vencida'` y esos umbrales no vuelven a matchear.
...
**UI (módulo 6 del rediseño, 2026-09-08):** el listado pasó de una grilla de tarjetas de cinco líneas (dos alumnas por pantalla) a **una fila por alumna** con inicial en círculo, apellido y grupo — buscador fijo arriba al scrollear, filtro de grupo por chips deslizables en vez del desplegable nativo, y letras como separador. DNI y fecha de inscripción salen del listado: es para encontrar a alguien, no para leer su ficha. El badge «Activa» se fue (lo dice el filtro); solo se marca la excepción: de baja, o las semanas seguidas sin venir. Activas/Bajas/Todas se repliegan detrás del contador. **La ficha** dejó de ser seis datos administrativos: arriba «Llamar a…» + WhatsApp al pagador principal, después la racha de faltas si la hay y dos recuadros —asistencia del mes y estado de la cuota, datos que el sistema ya tenía y la ficha no mostraba—, los contactos con botón de llamar cada uno, y al pie una línea con DNI, edad calculada e inscripción más «Editar datos». **Dar de baja salió del formulario** (ver `docs/decisiones.md`): vive en la ficha, pide `fecha_baja` y confirma, y avisa que la deuda no se borra. El **alta** arranca por lo que hace falta para una alumna que llegó hoy —nombre, grupo por chips y quién la trae con su teléfono—; DNI y las dos fechas se repliegan con sus valores por defecto a la vista. `src/lib/alumnas/ficha.ts` arma los datos de la ficha (`datosDeFichaAlumna`, `edadDe`) y las rachas del listado (`rachasParaListado`, una sola lectura para todo el club).

**UI (F2 MOD 2, reemplazada por lo de arriba):** listado en `/alumnas` (buscador + filtro de grupo + tabs de estado, todo client-side sobre las ~150 filas ya traídas del server, sin paginación ni búsqueda server-side), alta/edición en `/alumnas/nueva` y `/alumnas/[id]/editar` (contactos inline como repetidor de filas, único precedente de ese patrón en la app), ficha en `/alumnas/[id]`. Sección "Administración" nueva en la navegación (`AppHeader.tsx`), visible solo para Admin/Head Coach/Secretaria — único ítem por ahora es "Alumnas". Detalle completo en el historial de sesiones (`docs/historial/2026-09.md`).
...
**UI (módulo 5 del rediseño, 2026-09-08):** `/pagos` dejó de ser el menú de cuatro tarjetas y pasa a ser **el estado del mes** — recaudación como encabezado (solo Admin y Head Coach), y debajo lo que hay para hacer con el número adentro: pagos sin verificar, alumnas que deben, y «X de Y al día», que es la buena noticia cuando no hay deudoras. «Registrar» sube al encabezado porque es la acción del módulo, no una sección. El **navegador de mes es uno solo** (`NavegadorDeMes`, flechas de 36px): se elige el mes en la entrada y viaja como `?mes=` a las otras cuatro vistas, que antes repetían cada una sus propias flechas. `/pagos/deudoras` explica el recargo arriba y lo desglosa en cada monto («35.000 + 10.000») en vez de sumarlo mudo; «X días de atraso» sale del mes y no de la alumna, así que va una sola vez en el encabezado y cada línea dice **por qué** debe (no pagó / pagó parte / solo debe el recargo); las dos que más deben traen acción (WhatsApp al pagador y «Cobrar», que abre el alta ya resuelta) y el resto colapsa a una línea. `/pagos/nuevo` arranca por el dinero cuando viene de «Cobrar»: resumen de cuota + recargo + a cobrar, método y pagador por chips, y pie fijo con «Queda saldado» / «Quedan $X pendientes». `/pagos/pendientes` cambia «Marcar como verificado» por **«Está en el MP»** (dice qué se está afirmando), muestra cuánto lleva esperando cada pago y quién lo cargó, y manda el recibo por WhatsApp al pagador; abajo, los recibos de los pagos ya verificados del mes, para reenviar.

`src/lib/pagos/mes.ts` arma el estado del mes de la entrada; `src/lib/utils/whatsapp.ts` construye el link de `wa.me` **solo cuando el teléfono es reconocible** (10 dígitos locales, o ya con el 54) y devuelve `null` en cualquier otro caso — los teléfonos se cargan a mano y sin formato fijo, y vale más un botón que no aparece que uno que abre el chat equivocado; ahí la pantalla cae a copiar el texto.

**UI (F2 MOD 3, reemplazada por lo de arriba):** `/pagos` era un menú de cuatro tarjetas, `/pagos/nuevo` un formulario con buscador + checkbox de recargo abajo del scroll, `/pagos/deudoras` un listado con el recargo sumado sin avisar y el mismo «días de atraso» repetido en cada fila. `/pagos/recaudacion` (total del mes verificado + desglose por método) sigue igual salvo el navegador de mes. Sección "Administración" de `AppHeader.tsx` suma el ítem "Pagos" junto a "Alumnas".
...
F2 MOD 4 (2026-09-03). Una fila por alumna por fecha de clase.

**Guardado parcial desde el módulo 4 del rediseño (2026-09-08).** Se escribe **solo lo marcado**: una fila por alumna con «Vino» (`presente = true`) o «Faltó» (`presente = false`), y las que quedaron sin tocar no tienen fila. Desmarcar y volver a guardar borra la fila. Antes el guardado era en bloque (una fila por *cada* alumna activa del grupo, `presente = false` para toda la que no estuviera tildada), y eso convertía en ausente a la alumna que quien cargaba se olvidaba de tocar. **Sin migración**: «sin marcar» es la ausencia de fila, no un estado nuevo — el booleano `presente` no cambió.
...
**Estado de una fecha: se calcula, no se guarda.** Pendiente = sin ninguna marca; Parcial = tiene marcas pero menos que alumnas activas del grupo; Cargada = todas marcadas. Sin columna nueva: sale de contar las filas de `(grupo_id, fecha)` contra las alumnas activas (`src/lib/asistencia/dia.ts`).

**Alerta de inasistencias** (`src/lib/asistencia/alertas.ts`, cálculo por alumna en `src/lib/asistencia/rachas.ts`): **3 semanas calendario consecutivas sin ningún `presente`**, no 3 clases sueltas — contar clases daría un umbral distinto según el grupo, porque cada uno tiene su propia frecuencia semanal (2 o 3 días). La semana va de lunes a domingo y se recorre hacia atrás desde la actual, **sin contar la semana en curso** (para no disparar falsas alarmas a mitad de semana); si además la alumna ya tuvo un presente esta semana, queda fuera de la alerta (volvió). Una semana solo "cuenta" si **la alumna tiene filas propias** en ella: las semanas sin clase (feriados), sin cargar, o cargadas sin llegar a marcarla se saltean, no suman ni rompen la racha. Mirar sus filas y no las del grupo es lo que hace que el guardado parcial no invente ausencias. Es un reporte que se consulta (`/asistencia/alertas`), **sin notificación ni cron** — mismo criterio que "Deudoras" en Pagos.

**UI (módulo 4 del rediseño, 2026-09-08):** `/asistencia` es **el día**, no el menú del módulo — tira de días de lunes a sábado (el sábado en gris), las clases de esa fecha con su estado, y al pie las fechas atrasadas de la última semana, la alerta de inasistencias y el acceso al camino largo. `/asistencia/grupos` (el selector de grupo, que era la entrada del módulo) y `/asistencia/grupos/[grupoId]` (mes a mes, con los tres estados) quedan para buscar un mes viejo. `/asistencia/grupos/[grupoId]/[fecha]` es la toma: «Vino / Faltó» en botones de 44px, «Todas presentes» / «Limpiar», pie fijo con el conteo y cuántas faltan marcar, y el aviso de racha en la fila de la alumna desde las 2 semanas. `/asistencia/alertas` mantiene el mismo cálculo y orden, con el teléfono como botón «Llamar a…». Los grupos con clase en una fecha salen de `grupo_horarios`, no de `turnos`: la clase existe porque el grupo tiene ese día en su horario, exista o no la planificación.
...
RLS (verificada en vivo con transacciones con `rollback`, simulando cada rol): SELECT para Admin, Head Coach, Secretaria y Profesor; INSERT, UPDATE y DELETE para Admin, Head Coach y Secretaria. **Patinador/a y Empleado/a no figuran en ninguna policy**: la tabla no existe para ellos, tampoco entrando por URL directa (confirmado: una Empleada ve 0 filas). Confirmado también que una Profesora lee la lista pero su INSERT es rechazado (42501).

> **Corregido el 2026-09-08** (migración `20260908150000_secretaria_convoca_torneos.sql`, aplicada en `main` antes de abrir la rama de UI). El Bloque 5 había dejado INSERT y DELETE solo para Admin y Head Coach, con el criterio de que convocar era una decisión deportiva. En la práctica la lista la arma Secretaria, y quedaba a mitad de camino: podía marcar quién pagó pero no agregar a nadie. Ahora escribe la tabla entera igual que Admin y Head Coach. El SELECT no se tocó — sigue incluyendo a Profesor, que lee pero no modifica, y por eso las tres escrituras siguen en policies separadas en vez de una sola `for all`. La distinción vigente es entre tablas: `torneos` (el calendario) es Admin/Head Coach; `torneo_participantes` (la convocatoria) suma Secretaria.
