...
import { borrarTarea } from "@/app/(dashboard)/tareas/actions";
import { Spinner } from "@/components/ui/spinner";

export function BorrarTareaButton({ tareaId }: { tareaId: string }) {
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  function handleClick() {
    if (!confirm("¿Borrar esta tarea definitivamente? Esta acción no se puede deshacer.")) {
      return;
    }
    setError(null);
    startTransition(async () => {
      const result = await borrarTarea(tareaId);
      if (result?.error) {
