...
import { borrarTorneo } from "@/app/(dashboard)/torneos/actions";
import { Spinner } from "@/components/ui/spinner";

export function BorrarTorneoButton({ torneoId }: { torneoId: string }) {
  const [error, setError] = useState<string | null>(null);
  const [pending, startTransition] = useTransition();

  function handleClick() {
    if (!confirm("¿Borrar este torneo definitivamente? Esta acción no se puede deshacer.")) {
      return;
    }
    setError(null);
    startTransition(async () => {
      const result = await borrarTorneo(torneoId);
      if (result?.error) {
  ...
  return (
    <div className="space-y-1">
      ...
      >
        ...
        {pending ? "Borrando..." : "Borrar torneo"}
      </button>
      {error && (
